    ...
    TreeNode *buildTree(vector<int> preorder, vector<int> inorder)
    {
        ...
        for (int i = 0; i < inorder.size(); ++i)
        {
            if (preorder[0] == inorder[i])
            {
                ...
                node->left = buildTree(vector<int>(preorder.begin() + 1, preorder.begin() + i + 1), vector<int>(inorder.begin(), inorder.begin() + i));
                node->right = buildTree(vector<int>(preorder.begin() + i + 1, preorder.end()), vector<int>(inorder.begin() + i + 1, inorder.end()));
                break;
            }
        }
        return node;
